...
int points;
int multiplicadorZ = 100;
int multiplicadorF = 100;
...
Game::Game()
{
    ...
    if (!this->font.loadFromFile("alagard.ttf"))
        cout << "Error al cargar texto" << endl;
    this->puntos.setFont(font);
...
void Game::Update()
{
    ...
    if (getDistance(player.pos.x, player.pos.y, zombieSprite.getPosition().x, zombieSprite.getPosition().y) < 5 ||
        getDistance(player.pos.x, player.pos.y, fantasmaSprite.getPosition().x, fantasmaSprite.getPosition().y) < 5)
    {
...
void Game::mostrarpuntos()
{
    this->puntos.setPosition(0, 0);
    this->puntos.setString("puntaje: " + to_string(points));

    this->window->draw(puntos);
}
